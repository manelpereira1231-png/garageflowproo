...
BEGIN
  ...
  -- Contas de fornecedor (GSN) nunca criam oficina.
  IF COALESCE(NEW.raw_user_meta_data->>'account_type', '') IN ('supplier','gsn_supplier')
     OR COALESCE(NEW.raw_user_meta_data->>'supplier_user', 'false') = 'true' THEN
    RETURN NEW;
  END IF;

  IF NEW.email IS NOT NULL AND (
       EXISTS (
         SELECT 1 FROM public.gsn_supplier_invites i

  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (url) {
        ...
        throw new Error("URL de pagamento não recebido");
      }
    } catch (err: any) {
  ...
  if (!listing) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        <p className="text-lg">Anúncio não encontrado</p>
        <Link to="/carity"><Button>Voltar</Button></Link>
      </div>
    );
  }
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <ol className="space-y-2 text-sm text-muted-foreground">
                <li className="flex items-start gap-2">
                  <span className="bg-emerald-100 text-emerald-700 rounded-full w-5 h-5 flex items-center justify-center flex-shrink-0 text-xs font-bold">1</span>
                  Pagamento processado de forma segura via Stripe
                </li>
                <li className="flex items-start gap-2">
                  <span className="bg-emerald-100 text-emerald-700 rounded-full w-5 h-5 flex items-center justify-center flex-shrink-0 text-xs font-bold">2</span>
                  Uma oficina GarageFlow é atribuída automaticamente
                </li>
                <li className="flex items-start gap-2">
                  <span className="bg-emerald-100 text-emerald-700 rounded-full w-5 h-5 flex items-center justify-center flex-shrink-0 text-xs font-bold">3</span>
                  Inspeção completa com relatório detalhado
                </li>
                <li className="flex items-start gap-2">
                  <span className="bg-emerald-100 text-emerald-700 rounded-full w-5 h-5 flex items-center justify-center flex-shrink-0 text-xs font-bold">4</span>
                  Carro publicado no marketplace após aprovação
                </li>
              </ol>
            </div>
            ...
            <p className="text-xs text-center text-muted-foreground">
              Pagamento seguro processado por Stripe. Pode cancelar a qualquer momento.
            </p>
          </CardContent>
        </Card>
      </div>

...
export default function QuoteForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!clientId || !vehicleId || lines.length === 0) {
      toast.error("Preencha cliente, veículo e adicione pelo menos uma linha.");
      return;
    }
    setLoading(true);
    ...
    <div className="max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={vehicleId} onValueChange={setVehicleId} disabled={!clientId}>
                <SelectTrigger><SelectValue placeholder={clientId ? "Selecionar" : "Escolha cliente primeiro"} /></SelectTrigger>
                <SelectContent>
        ...
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          ...
          {lines.length === 0 && (
            <p className="text-muted-foreground text-sm text-center py-4">Adicione serviços ou peças ao orçamento.</p>
          )}
          ...
          {lines.map((line, i) => (
            <div key={line.id} className="grid grid-cols-12 gap-2 items-end border-b border-border pb-3">
              ...
              <div className="col-span-3 sm:col-span-1">
                ...
                <Input className="h-9 text-sm" type="number" min={1} value={line.quantity} onChange={e => updateLine(line.id, 'quantity', +e.target.value)} />
              </div>
              <div className="col-span-3 sm:col-span-2">
                <Label className="text-xs">Preço</Label>
                <Input className="h-9 text-sm" type="number" step="0.01" value={line.unit_price} onChange={e => updateLine(line.id, 'unit_price', +e.target.value)} />
              </div>
              <div className="col-span-3 sm:col-span-2">
                <Label className="text-xs">Custo</Label>
                <Input className="h-9 text-sm" type="number" step="0.01" value={line.unit_cost} onChange={e => updateLine(line.id, 'unit_cost', +e.target.value)} />
              </div>
              <div className="col-span-2 sm:col-span-1">
                <Label className="text-xs">IVA%</Label>
                <Input className="h-9 text-sm" type="number" value={line.vat_rate} onChange={e => updateLine(line.id, 'vat_rate', +e.target.value)} />
              </div>
              <div className="col-span-1 sm:col-span-1 text-right">
                <Label className="text-xs">Total</Label>
                <p className="mono text-sm font-medium h-9 flex items-center justify-end">€{(line.quantity * line.unit_price).toFixed(2)}</p>
              </div>
              <div className="col-span-12 sm:col-span-1 flex justify-end">
                <Button type="button" variant="ghost" size="icon" className="h-9 w-9 text-destructive" onClick={() => removeLine(line.id)}>
        ...
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          ...
          <div className="space-y-1.5">
            <Label>Notas</Label>
            <Textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Observações para o cliente..." />
          </div>
        </div>

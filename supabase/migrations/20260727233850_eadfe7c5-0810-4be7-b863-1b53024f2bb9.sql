...
BEGIN
  ...
  -- Country_code chosen at signup wins. Fallback to legacy shop_country name,
  -- else PT. This is the SINGLE point where a shop's country is set.
  ...
  SELECT code, name, currency, default_language, tax_label,
         COALESCE((timezones->>0), NULL) AS tz
    INTO v_cs

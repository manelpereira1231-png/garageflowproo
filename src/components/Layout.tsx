...
const ONBOARDING_COMPLETED_KEY = "garageflow_onboarding_completed";
const ONBOARDING_STATE_EVENT = "garageflow:onboarding-state-changed";
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const checkOnboardingProgress = async () => {
      ...
      if (!isReady || !user || localStorage.getItem(ONBOARDING_COMPLETED_KEY) === "true") {
        return;
      }
    ...
    window.addEventListener(ONBOARDING_STATE_EVENT, syncOnboardingState as EventListener);
    window.addEventListener("storage", syncOnboardingState);

    return () => {
      window.removeEventListener(ONBOARDING_STATE_EVENT, syncOnboardingState as EventListener);
      window.removeEventListener("storage", syncOnboardingState);
    };
  }, [activeShopId, isReady, user, location.pathname]);

  useEffect(() => {
    if (isOnboardingMode && !isEssentialPath(location.pathname) && !isFinancialRoute(location.pathname)) {
      setShowAllFeatures(true);
    }
  }, [isOnboardingMode, location.pathname]);
  ...
  const shouldShowSimplifiedMenu = isOnboardingMode && !showAllFeatures;
  const visibleNavItems = shouldShowSimplifiedMenu
  ...
  const simpleModeHint =
    t("nav.simpleStartHint") === "nav.simpleStartHint"
      ? "Mostramos só o essencial até criares cliente, veículo e orçamento."
      : t("nav.simpleStartHint");
  const showAllLabel =
    t("nav.showAllFeatures") === "nav.showAllFeatures"
      ? "Ver todas as funcionalidades"
      : t("nav.showAllFeatures");
  const showEssentialLabel =
    t("nav.showEssentialOnly") === "nav.showEssentialOnly"
      ? "Mostrar só o essencial"
      : t("nav.showEssentialOnly");
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-2.5 space-y-0.5">
          {isOnboardingMode && (
            <div className="mb-3 rounded-xl border border-sidebar-border bg-sidebar-accent/40 p-2.5">
              ...
              >
                <span>{shouldShowSimplifiedMenu ? showAllLabel : showEssentialLabel}</span>
                <ChevronDown className={`ml-auto w-3.5 h-3.5 transition-transform ${showAllFeatures ? "rotate-180" : ""}`} />
              </button>
            </div>
          )}

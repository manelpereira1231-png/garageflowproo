...
Deno.serve(async (req) => {
  ...
  let from = Deno.env.get("TWILIO_WHATSAPP_FROM") || "";
  if (from && !from.startsWith("whatsapp:")) from = `whatsapp:${from}`;

  if (!LOVABLE_API_KEY || !TWILIO_API_KEY || !from) {
    return new Response(
      JSON.stringify({ error: "WhatsApp channel not configured" }),
  ...
  try {
    ...
    if (!resp.ok) {
      console.error("Twilio WA error", resp.status, text);
      return new Response(
    ...
    if (shop_id) {
      await supabase.from("email_logs").insert({
        shop_id, to_email: toWa, subject: `[WhatsApp] ${message.slice(0, 60)}`,
        status: "sent", entity_type: "whatsapp", entity_id: entity_id ?? null,
      });
    }

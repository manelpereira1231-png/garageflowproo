...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const NAV_GROUPS: { id: string; label: string; paths: string[] }[] = useMemo(() => [
    { id: "ops", label: t("navGroup.ops", "Operação Diária"), paths: ["/clients","/vehicles","/fleets","/quotes","/services","/workshop","/agenda","/claims"] },
  ...
  return (
    <div className="h-screen flex w-full bg-background overflow-hidden">
      ...
      >
        ...
        <div className={`h-14 lg:h-16 flex items-center border-b border-sidebar-border shrink-0 ${sidebarCompact ? "px-2 justify-center" : "px-2 lg:px-3"}`}>
          <div className="flex flex-1 items-center gap-2.5 min-w-0">
            ...
            {!sidebarCompact && (
              <span className="text-lg font-bold text-sidebar-accent-foreground tracking-tight whitespace-nowrap shrink-0">
      ...
      <main className="flex-1 flex flex-col h-screen min-w-0 overflow-y-auto overflow-x-hidden">
        ...
        <div className={`flex-1 page-in pb-10 sm:pb-8 ${isServicesListRoute ? "p-2 sm:p-3 lg:p-4" : "p-3 sm:p-4 lg:p-6"}`}>

...
async function processGeneration(opts: {
  creativeId: string;
  userId: string;
  model: string;
  prompt: string;
  ...
  try {
    ...
    if (!imgResp.ok) {
      ...
      await supa.from("marketing_creatives").update({
        status: "failed",
        error: `${imgResp.status}: ${errText.slice(0, 300)}`,
    ...
    const storagePath = `${opts.userId}/${opts.creativeId}.png`;
    ...
    await supa.from("marketing_creatives").update({
      status: "failed",
      error: e?.message?.slice(0, 300) ?? "Erro desconhecido",
    }).eq("id", opts.creativeId);
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Background: gera + upload sem bloquear a resposta (evita timeout para qualidade alta)
    const work = processGeneration({
    ...
    return json({ ok: true, queued: true, creativeId: created.id, status: "generating" }, 202);
  } catch (e: any) {

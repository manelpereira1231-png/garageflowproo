...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {chooserEnabled && erpEnabled === true && marketEnabled === true && (() => {
        ...
        return (
          <aside
            ...
            className="pt-20 sm:pt-24 pb-6 sm:pb-8 px-4 relative"
          >
            {/* subtle gradient background to separate from hero */}
                ...
                {cards.map((c) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`group relative flex items-center gap-4 rounded-2xl border ${ring} bg-card/80 backdrop-blur-sm transition-all px-5 py-4 sm:px-6 sm:py-5`}
                    >
      ...
      {/* Hero */}
      <header className={`${chooserEnabled && erpEnabled === true && marketEnabled === true ? "pt-12 sm:pt-16" : "pt-[116px] sm:pt-[128px] lg:pt-36"} pb-16 sm:pb-20 px-4 text-center relative overflow-hidden`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            <div>
              ...
              <ul className="space-y-2 text-sm">
                {bullets.map((b) => (
                  <li key={b} className="flex gap-2"><Check className="w-4 h-4 text-primary shrink-0 mt-0.5" /> {b}</li>
                ))}
              </ul>
            </div>

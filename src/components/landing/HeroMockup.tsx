 ...
 * Replaces the previous AI-generated image — lighter, sharper, brand-accurate.
 */
export default function HeroMockup() {
  return (
    <div className="relative rounded-2xl overflow-hidden border border-border shadow-2xl bg-card aspect-[16/10]">
      {/* Window chrome */}
      <div className="flex items-center gap-1.5 px-3 py-2 border-b border-border bg-muted/40">
        <span className="w-2.5 h-2.5 rounded-full bg-red-400/70" />
        <span className="w-2.5 h-2.5 rounded-full bg-yellow-400/70" />
        <span className="w-2.5 h-2.5 rounded-full bg-green-400/70" />
        <div className="ml-3 px-2.5 py-0.5 rounded-md bg-background text-[10px] text-muted-foreground font-mono">
          app.garageflow.pt/dashboard
        </div>
      </div>

      <div className="grid grid-cols-[180px_1fr] h-[calc(100%-32px)]">
        {/* Sidebar */}
        ...
        {/* Main */}
        <main className="p-3 sm:p-4 overflow-hidden">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm sm:text-base font-bold">Painel da oficina</h3>
          ...
          <div className="grid grid-cols-4 gap-2 mb-3">
            {[
              ...
              <div key={k.l} className="rounded-lg border border-border bg-background p-2">
                <p className={`text-base sm:text-lg font-bold ${k.c}`}>{k.v}</p>
                <p className="text-[9px] sm:text-[10px] text-muted-foreground">{k.l}</p>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-lg border border-border bg-background overflow-hidden">
            ...
            {[
              ...
              <div key={row.c} className="flex items-center justify-between px-2.5 py-1.5 border-b border-border/50 last:border-0">
                <div className="min-w-0">
                  <p className="text-[11px] font-medium truncate">{row.c}</p>
                  <p className="text-[10px] text-muted-foreground truncate">{row.v}</p>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full ${row.color}`}>{row.s}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Floating phone — WhatsApp preview */}
      <div className="hidden md:block absolute -bottom-6 -right-4 w-44 h-72 rounded-[20px] border-4 border-foreground/80 bg-background shadow-2xl rotate-[6deg] overflow-hidden">
        ...
        <div className="p-2 space-y-1.5 bg-[#ECE5DD] h-[calc(100%-52px)]">
          <div className="bg-white rounded-lg p-1.5 max-w-[80%] shadow-sm">
            <p className="text-[9px] text-foreground">Orçamento #1042 pronto: €245</p>
          </div>
          <div className="bg-[#DCF8C6] rounded-lg p-1.5 max-w-[80%] shadow-sm ml-auto">
            <p className="text-[9px] text-foreground flex items-center gap-1">Aprovado <CheckCircle2 className="w-2.5 h-2.5 text-green-600" /></p>
          </div>
          <div className="bg-white rounded-lg p-1.5 max-w-[80%] shadow-sm">
            <p className="text-[9px] text-foreground">Carro pronto sexta às 15h ✅</p>
          </div>
        </div>
      </div>
    </div>

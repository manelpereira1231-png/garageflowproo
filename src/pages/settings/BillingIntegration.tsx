...
// Human-readable provider metadata (single source of truth: billing/providers.ts).
import { PROVIDERS } from "@/lib/billing/providers";

const PROVIDER_LABEL: Record<string, string> = Object.fromEntries(
  Object.values(PROVIDERS).map((p) => [p.slug, p.name]),
);
const PROVIDER_URL: Record<string, string> = Object.fromEntries(
  Object.values(PROVIDERS)
    .filter((p) => !!p.url)
    .map((p) => [p.slug, p.url as string]),
);
const PROVIDER_DOC_TYPES: Record<string, { value: string; label: string }[]> = Object.fromEntries(
  Object.values(PROVIDERS).map((p) => [p.slug, p.docTypes]),
);
